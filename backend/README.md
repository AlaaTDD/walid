...
- الصور (المرفوعة، الباقية، وملف TIFF النهائي) تُحفظ تلقائيًا تحت `images/<job_id>/` بجانب تطبيق العميل نفسه (مسار ثابت واحد فقط، لا يوجد إعداد مسار من طرف العميل). راجع `app/api/job_storage.py`'s `ensure_images_job_dir`.
